...
def run_molecule() -> None:
    ...
    energy_data_val_and_grad = physics.core.create_double_chain_energy_val_and_grad_fn(
        psi_apply, standard_local_energy_fn, nchains=config.vmc.nchains
    )



    pretrain_energy_data_val_and_grad = physics.core.create_value_and_grad_energy_fn(
        log_psi_apply,
        standard_local_energy_fn,
        nchains=config.vmc.nchains,
    )

    def pretrain_iteration(data, grad_data, key, wf_opt_state, wf_params):
        ...
        (energy, aux_data), grad_e = pretrain_energy_data_val_and_grad(wf_params, key, position)
        variance = aux_data[0]
    ...
    pretrain_iteration = jax.jit(pretrain_iteration)
    ...
    fpath = os.path.join(logdir, "pretrain.txt")
    ...
    logging.info("Completed pretraining! Starting real training.")
    ...
    def wf_train_iteration(data, grad_data, key, wf_opt_state, wf_params):
        ...
        position = data["walker_data"]["position"]
        grad_position = grad_data["walker_data"]["position"]
        (energy, variance, mean_inv_abs_psi), grad_e = energy_data_val_and_grad(
            wf_params, position, grad_position
        )
        norm_grad = jnp.linalg.norm(jax.flatten_util.ravel_pytree(grad_e)[0])
        updates, wf_opt_state = wf_opt.update(grad_e, wf_opt_state)
        wf_params = optax.apply_updates(wf_params, updates)
    ...
    with open(fpath, "w") as f:
        for i in range(config.vmc.nepochs):
            ...
            logging.info(
                f"Epoch {i:5d}   Energy {energy:3e}   "
                f"Variance {variance:3e}   Scale factor: {mean_inv_abs_psi:3e}   "
                f"Norm grad: {norm_grad:3e}   Accept ratio: {accept_ratio:3f}   "
                f"Grad accept ratio: {grad_accept_ratio:3f}"
            )
            f.write(f"{energy} {variance}\n")
