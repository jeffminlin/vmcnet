...
def create_value_and_grad_energy_fn(
    ...
    def energy_val_and_grad(params, positions):
        def importance_local_energy(pa, pos):
            return local_energy_fn(pa, pos) / (1 + jnp.abs(ei_potential_fn(pa, pos)))

        val_and_grad_local_energy = jax.value_and_grad(
            importance_local_energy, argnums=0
        )
        ...
        aux_data, energy = calculate_statistics(
            local_energies_noclip / scale_correction
        )
    ...
    def calculate_statistics(local_energies_noclip):
        if clipping_fn is not None:
            ...
            local_energies_noclip = clipping_fn(local_energies_noclip, energy_noclip)
