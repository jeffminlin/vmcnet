...
def initialize_spring_diag(
    ...
    spring_step = get_spring_step(
        ...
        optimizer_config.preconditioner_type,
    )
    ...
    def optimizer_apply(energy, local_energies, params, optimizer_state, data):
        ...
        diag_B = optimizer_state[2]
        grad, diag_B = spring_step(
        ...
        optimizer_state = [*optimizer_state[:2], diag_B]
    ...
    optimizer_state = [*optimizer_state, jnp.ones(nparams)]
...
def get_spring_step(
    ...
    def spring_step(
        ...
        if preconditioner_type == "fisher":
            diag_B = beta * diag_B + (1- beta) * jnp.mean(Ohat**2, axis=0)
            diag_P = diag_B
        elif preconditioner_type == "march":
            diag_B = beta * diag_B + (1- beta) * prev_grad**2
            diag_P = jnp.sqrt(diag_B)
        elif preconditioner_type == "march_fisher":
            diag_B = beta * diag_B + (1- beta) * prev_grad**2
            diag_P = diag_B
