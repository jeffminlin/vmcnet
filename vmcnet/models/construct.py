...
class FermiNetSurrogate(Module):
    ...
    @flax.linen.compact
    def __call__(self, elec_pos: Array, psi_inverse) -> Array:  # type: ignore[override]
        """Compose FermiNet backflow -> orbitals -> logabs determinant product.
        ...
        smoothed_psi_inverse = (
            jnp.expand_dims(psi_inverse, axis=-1)
            + jnp.expand_dims(jnp.sign(psi_inverse), axis=-1) * sign_psi_weight
        )
